...
int main( int argc, char* argv[] )
{
    ...
    // 解析配置文件，生成向量列表（手动解析，硬核、脆弱、不安全，可以优化）
    ...
    while( (tmp2 = strpbrk( tmp, "\n" )) != NULL )
    {
        *tmp2++ = '\0';
        ...
        else if( (tmp3 = strstr( tmp, "<name>" )) != NULL )
        {
            tmp_hostname = tmp3 + 6;
        ...
        else if( (tmp3 = strstr( tmp, "<port>" )) != NULL )
        {
            tmp_port = tmp3 + 6;
        ...
        else if( (tmp3 = strstr( tmp, "<conns>" )) != NULL )
        {
            tmp_conncnt = tmp3 + 7;
